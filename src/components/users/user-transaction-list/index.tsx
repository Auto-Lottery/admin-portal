...
const UserTransactionList = ({ userData }: { userData: { userId: string, phoneNumber: string } }) => {
    ...
    const getTransactionList = useCallback(async (page: number, pageSize: number) => {
        const res = await getRequest(`/transaction/adminTransaction/getUserTransactions?page=${page}&pageSize=${pageSize}&phoneNumber=${userData.phoneNumber}`);
